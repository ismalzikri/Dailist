...
function App() {
  ...
  const [todoItems, setTodoItems] = useState([
    { id: 1, title: "inisiate project & setup", finished: false },
    { id: 2, title: "create UI with html & css", finished: true },
    { id: 3, title: "Seperate element into a small components", finished: true },
    { id: 4, title: "inisiate project & setup", finished: true },
    { id: 5, title: "create UI with html & css", finished: true },
    { id: 6, title: "inisiate project & setup", finished: true },
    { id: 7, title: "create UI with html & css", finished: true },
  ])
      ...
      <div className="wrapper">
        ...
        <div className="wrapper__upcoming">
          ...
          <ul className="wrapper__upcoming__items">
            {todoItems.filter((todoItem) => todoItem.finished === false).map((todoItem, index) => {
              return <li key={todoItem.id}>
                <span>0{index + 1}. {todoItem.title}</span>
                <svg
                  width={32}
                  height={32}
        ...
        <div className="wrapper__finished">
          ...
          <ul className="wrapper__finished__items">
            {todoItems.filter((todoItem) => todoItem.finished === true).map((todoItem, index) => {
              return <li key={todoItem.id}>
                <span>0{index + 1}. {todoItem.title}</span>
              </li>
            })}
        ...
        <footer>
          ...
          {isModalOpen && <Modal handleSubmitTodo={handleSubmitTodo} onClose={handleToggleModal} />}
        </footer>
      </div>
    </main>
  );
